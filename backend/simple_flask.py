...
# AI API functions
def call_openai_sync(prompt: str, max_tokens: int = 500, temperature: float = 0.7):
    try:
...
def call_google_sync(prompt: str, max_tokens: int = 500, temperature: float = 0.7):
    try:
...
def compare_ai_responses():
    try:
        ...
        if "openai" in providers:
            result = call_openai_sync(prompt, max_tokens, temperature)
            results.append({
                "provider": "openai",
        ...
        if "google" in providers:
            result = call_google_sync(prompt, max_tokens, temperature)
            results.append({
                "provider": "google",
                "response": result["response"],
                "timestamp": datetime.now().isoformat(),
...
if __name__ == "__main__":
    logger.info("Starting AnythingLibrary Simple Flask API...")
